...
def app():
    ...
    st.markdown("<p style='color: gold; font-size: 18px;'>Find your news update here</p>", unsafe_allow_html=True)
    ...
    st.header("Top News")
    # Display the top 5 news articles with clickable titles and collapsible bodies
    top_articles = data.sample(5)
    for index, row in top_articles.iterrows():
        # Use a unique key for each expander based on the index
        with st.expander(f"{row['title']}_{index}"):
            st.title(row['title'])
            # Justify text and add paragraphs to the body
            justified_and_paragraphs_text = justify_and_paragraphs(row['body'])
            st.markdown(justified_and_paragraphs_text, unsafe_allow_html=True)  # Using markdown to render HTML content
        
    st.header("Search for Article")
